...
public class ProfilePageSteps extends PageObject {
    ...
    @Given("Verify added book")
    public void verifyAddedBook() {
        List<BookModel> bookList = ((UserResponseModel) ContextStore.get("userResponse")).getBooks();

        profilePage.waitForList(profilePage.bookTitles);
        for (BookModel book : bookList) {
            String expectedPages = book.getIsbn();
            String  actualPages = "9781449325862";
            Assert.assertEquals("Book is not on the list!", expectedPages, actualPages);
        }
